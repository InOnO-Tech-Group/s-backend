...
export const isBlogAlreadyExists = async (req, res, next) => {
  try {
    const article = await blogRepository.findBlogByAttribute(
      "title",
      req.body.title
    );
    if (article) {
...
export const isAnyPublishedBlog = async (req, res, next) => {
  try {
    const blogs = await blogRepository.findPublishedBlogs();
